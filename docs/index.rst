...
If argument ``-o, --obj OBJ`` is specifed, then it overrules ``OBJ`` and ``HDLMAKE_USE_OBJ``.
...
``-o, --obj OBJ``
-------------------------------
If specified, then compilation obj-files are store in a subfolder hierarchy to ``OBJ``. The subfolder hierarchy is the root folder of the project.
To let the path be valid modified, eg ``:`` is removed on Windows.

Can also be set via Environment variable ``OBJ`` and ``HDLMAKE_USE_OBJ``.
